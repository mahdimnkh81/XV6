...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(winner){
      ...
      acquire(&tickslock);
      winner->last_ran = ticks;
      release(&tickslock);

      swtch(&c->context, &winner->context);

      acquire(&tickslock);
      winner->ticks += ticks - winner->last_ran;
      release(&tickslock);
...
getpinfo(uint64 addr)
{
  ...
  // check if address can hold a pstat struct
  if (addr + sizeof(struct pstat) > myproc()->sz) {
    printf("addr + sizeof(struct pstat) > myproc()->sz\n");
    return -1;
  }
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if (p->state != UNUSED) {
      ...
      acquire(&tickslock);
      pstat.ticks[i] = p->state == RUNNING ? (ticks - p->last_ran) + p->ticks : p->ticks;
      release(&tickslock);
      ++i;
    }
    release(&p->lock);
  ...
  // copy pstat to user space
  if (copyout(myproc()->pagetable, addr, (char*)&pstat, sizeof(struct pstat)) < 0) {
